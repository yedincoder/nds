...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge-archived { background: #e2e3e5; color: #6c757d; }
    </style>
</head>
        ...
        <div class="main-content">
            ...
            <main class="content" style="padding: 30px;">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h1 class="section-title mb-0">Manage Articles</h1>
                    <a href="/admin/cms/articles/create" class="btn btn-primary"><i class="fas fa-plus"></i> Add Article</a>
                </div>
                <div class="card">
                                    ...
                                    <?php if (!empty($articles)): ?>
                                        <?php foreach ($articles as $i => $article): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="badge-status badge-<?= esc($article->status ?? 'pending') ?>"><?= esc($article->status ?? 'pending') ?></span>
                                                </td>
                                                <td><?= esc($article->created_at ?? '-') ?></td>
                                                <td>
                                                    <a href="/admin/cms/articles/<?= esc($article->id) ?>/edit" class="btn btn-sm btn-outline-primary">Edit</a>
                                                    <a href="/admin/cms/articles/<?= esc($article->id) ?>/delete" class="btn btn-sm btn-outline-danger">Delete</a>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php else: ?>
