...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge-published { background: #d4edda; color: #15803d; }
        .badge-featured { background: #fff3cd; color: #856404; }
        .badge-draft { background: #e2e3e5; color: #6c757d; }
        .portfolio-img { width: 60px; height: 60px; object-fit: cover; border-radius: 8px; }
    </style>
</head>
        ...
        <div class="main-content">
            ...
            <main class="content" style="padding: 30px;">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h1 class="section-title mb-0">Manage Portfolio</h1>
                </div>
                <div class="card">
                                                ...
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                        <?php if (!empty($portfolio->image)): ?>
                                                            <img src="<?= esc($portfolio->image) ?>" alt="<?= esc($portfolio->title) ?>" class="portfolio-img me-3">
                                                        <?php endif; ?>
                                                        <strong><?= esc($portfolio->title ?? $portfolio->name) ?></strong>
                                                    </div>
                                                </td>
                                                <td>
                                                    <span class="badge-status badge-<?= esc($portfolio->status ?? 'draft') ?>"><?= esc($portfolio->status ?? 'draft') ?></span>
                                                </td>
                                                <td><?= esc($portfolio->created_at ?? '-') ?></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        <tr><td colspan="4" class="text-center text-muted py-4">No portfolio items found</td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
