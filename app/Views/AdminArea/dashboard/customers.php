...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .table th { background: var(--header-bg); font-weight: 600; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px; }
        .status-badge { font-size: 11px; padding: 4px 10px; border-radius: 20px; font-weight: 600; }
        .badge-active { background: #d4edda; color: #15803d; }
        .badge-inactive { background: #f8d7da; color: #dc3545; }
    </style>
</head>
<body data-active="customers">
        ...
        <div class="main-content">
            ...
            <main class="content" style="padding: 30px;">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h1 class="section-title mb-0">Manage Customers</h1>
                </div>
                <div class="card">
                                    ...
                                    <?php if (!empty($customers)): ?>
                                        <?php foreach ($customers as $i => $customer): ?>
                                            <tr>
                                                ...
                                                <td><?= esc($customer->id ?? $customer->user_id ?? $customer->user_profiles_id) ?></td>
                                                <td><?= esc($customer->username ?? $customer->user_username ?? $customer->username ?? '-') ?></td>
                                                <td><?= esc($customer->email ?? $customer->user_email ?? $customer->email ?? '-') ?></td>
                                                <td>
                                                    <span class="status-badge status-<?= esc($customer->status ?? 'active') ?>"><?= esc($customer->status ?? 'active') ?></span>
                                                </td>
                                                <td><?= esc($customer->created_at ?? $customer->user_profiles_created_at ?? '-') ?></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php else: ?>
