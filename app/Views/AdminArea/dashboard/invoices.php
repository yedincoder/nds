...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge-paid { background: #d4edda; color: #15803d; }
        .badge-unpaid { background: #f8d7da; color: #dc3545; }
    ...
    <div class="shell d-flex">
        <aside class="sidebar" id="sidebar">
            ...
            <nav class="nav-section">
                ...
                <a class="nav-link active" href="/admin/invoices"><i class="fas fa-file-invoice"></i><span>Invoices</span></a>
        ...
        <div class="main-content">
            ...
            <main class="content" style="padding: 30px;">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h1 class="section-title mb-0">Manage Invoices</h1>
                </div>
                <div class="card">
                                    ...
                                    <?php if (!empty($invoices)): ?>
                                        <?php foreach ($invoices as $i => $invoice): ?>
                                            <tr>
                                                ...
                                                <td><?= esc($invoice->invoice_number ?? $invoice->id) ?></td>
                                                <td><?= esc($invoice->username ?? 'Guest') ?></td>
                                                <td>Rp <?= number_format($invoice->total ?? 0) ?></td>
                                                <td>
                                                    <span class="badge-status badge-<?= esc($invoice->status ?? 'pending') ?>"><?= esc($invoice->status ?? 'pending') ?></span>
                                                </td>
